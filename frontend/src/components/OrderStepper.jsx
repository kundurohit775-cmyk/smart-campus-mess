    ...
    return (
      <div className="flex items-center gap-3 p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-800">
        ...
        <div>
          ...
          <p className="text-xs text-rose-600">Credits were refunded back to student balance.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full py-4">
      <div className="relative flex items-center justify-between">
        ...
        <div className="absolute top-1/2 left-0 right-0 -translate-y-1/2 h-1.5 bg-slate-100 rounded-full z-0 mx-6">
          <div
        ...
        {STEPS.map((step, idx) => {
          ...
          if (isDone) {
            nodeStyle = 'bg-emerald-500 border-emerald-500 text-white shadow-sm';
            textColor = 'text-emerald-700 font-semibold';
          } else if (isCurrent) {
            nodeStyle = 'bg-orange-500 border-orange-500 text-white ring-4 ring-orange-100 shadow-md animate-pulse-subtle';
            textColor = 'text-orange-600 font-bold';
          }

          return (
            <div key={step.key} className="relative z-10 flex flex-col items-center group">
              <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${nodeStyle}`}>
                <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <div className="mt-2 text-center hidden sm:block">
                <p className={`text-xs ${textColor}`}>{step.label}</p>
                <p className="text-[10px] text-slate-400 max-w-[80px] leading-tight">{step.desc}</p>
              </div>
            </div>
          );
        })}
